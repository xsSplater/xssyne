...
func NewThemeOverride(obj fyne.CanvasObject, th fyne.Theme) *ThemeOverride {
	...
	// тема применяется в CreateRenderer, здесь достаточно первичного Refresh,
	// чтобы виджеты подхватили новую тему после рендера.
	cache.OverrideTheme(obj, addFeatures(th, t))
	return t
}

func (t *ThemeOverride) CreateRenderer() fyne.WidgetRenderer {
	// повторно не переопределяем — это уже сделано в конструкторе.
	// если Content был подменён через Refresh, там же и переопределяется.
