...
// Дополнительно добавлен ранний выход, если окно уже наверху — не дёргаем refreshChildren зря.
func (m *MultipleWindows) RaiseToTop(w *InnerWindow) {
	...
	if id == -1 || id == len(m.Windows)-1 {
		return
	}

	windows := make([]*InnerWindow, 0, len(m.Windows))
	windows = append(windows, m.Windows[:id]...)
	windows = append(windows, m.Windows[id+1:]...)
