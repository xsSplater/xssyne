...
// selectIndex умышленно не умеет сбрасывать выбор в -1, поэтому делаем это явно. Заодно убираем двойную проверку selected(t) == item и ранний возврат.
func disableIndex(t baseTabs, index int) {
	...
	// no other tab is able to be selected
	if f := t.onUnselected(); f != nil && item != nil {
		f(item)
	}
	t.setSelected(-1)
	t.Refresh()
