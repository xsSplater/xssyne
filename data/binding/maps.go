...
func (b *boundStruct) Reload() (retErr error) {
	...
	for j := 0; j < v.NumField(); j++ {
		...
		kind := f.Kind()
		switch kind {
		case reflect.Slice, reflect.Struct, reflect.Map, reflect.Func,
			reflect.Chan, reflect.Pointer, reflect.UnsafePointer:
			fyne.LogError("Data binding does not yet support "+kind.String()+" elements in a struct", nil)
			continue
		}

		key := t.Field(j).Name
		old := (*b.val)[key]
		if reflect.DeepEqual(f.Interface(), old) {
			continue
		}
