...
func (f *fileDialog) makeUI() fyne.CanvasObject {
	...
	verticalExtra := float32(float64(theme.IconInlineSize()) * 0.25)
...
func (f *fileDialog) makeOpenButton(label string) *widget.Button {
	btn := widget.NewButton(label, func() {
		...
		if f.file.save {
			callback, ok := f.file.callback.(func(fyne.URIWriteCloser, error))
			if !ok {
				fyne.LogError("FileDialog: unexpected callback type for save mode", nil)
				f.win.Hide()
				return
			}
			...
			callback, ok := f.file.callback.(func(fyne.URIReadCloser, error))
			if !ok {
				fyne.LogError("FileDialog: unexpected callback type for open mode", nil)
				f.win.Hide()
				return
			}
			f.win.Hide()
			...
			callback, ok := f.file.callback.(func(fyne.ListableURI, error))
			if !ok {
				fyne.LogError("FileDialog: unexpected callback type for folder mode", nil)
				f.win.Hide()
				return
			}
			f.win.Hide()
...
func (f *fileDialog) makeDismissButton(label string) *widget.Button {
	btn := widget.NewButton(label, func() {
		...
		if f.file.save {
			if cb, ok := f.file.callback.(func(fyne.URIWriteCloser, error)); ok {
				cb(nil, nil)
			} else {
				fyne.LogError("FileDialog: unexpected callback type for save mode", nil)
			}
		} else if f.file.isDirectory() {
			if cb, ok := f.file.callback.(func(fyne.ListableURI, error)); ok {
				cb(nil, nil)
			} else {
				fyne.LogError("FileDialog: unexpected callback type for folder mode", nil)
			}
		} else {
			if cb, ok := f.file.callback.(func(fyne.URIReadCloser, error)); ok {
				cb(nil, nil)
			} else {
				fyne.LogError("FileDialog: unexpected callback type for open mode", nil)
			}
