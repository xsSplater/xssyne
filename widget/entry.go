...
type Entry struct {
	...
	// EscapeHandler is called when the Escape key is pressed while the entry has focus.
	// If set, it will be called instead of losing focus or performing any other default action.
	escapeHandler func()
}
...
// SetEscapeHandler sets a function to be called when the Escape key is pressed
// while this entry has focus. This overrides the default behavior of losing focus.
func (e *Entry) SetEscapeHandler(f func()) {
	e.escapeHandler = f
}
...
func (e *Entry) TypedKey(key *fyne.KeyEvent) {
	...
	// Обработка Escape, если установлен обработчик
	if key.Name == fyne.KeyEscape && e.escapeHandler != nil {
		e.escapeHandler()
		return
	}
	if e.cursorAnim != nil {
