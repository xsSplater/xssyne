...
func (i *Image) imageDetailsFromReader(source io.Reader) (io.Reader, int, int, float32, error) {
	if source == nil {
		return nil, 0, 0, 0, errors.New("no matching reading reader")
	}

	if i.isSVG {
		icon, err := svg.NewDecoder(source)
		if err != nil {
			return nil, 0, 0, 0, err
		}
		i.icon = icon
		config := icon.Config()
		// For SVG, reader is intentionally nil — the decoder consumes the source.
		return nil, config.Width, config.Height, config.Aspect, nil
	}
	...
	return reader, config.Width, config.Height, float32(config.Width) / float32(config.Height), nil
}

func (i *Image) renderSVG(width, height float32) (image.Image, error) {
	var c fyne.Canvas
	if app := fyne.CurrentApp(); app != nil && app.Driver() != nil {
		c = app.Driver().CanvasForObject(i)
	}
